import type { CelestialBodyConfig } from '../types';

// Phase 1: Sun + 8 planets (9 bodies)
// Audio stems are placeholders — will be replaced with composed stems
// Audibility radius scaled roughly by body size (larger = audible from further)

export const BODIES: CelestialBodyConfig[] = [
  {
    id: 'sun',
    name: 'Sun',
    type: 'star',
    radiusKm: 696_340,
    audibilityRadiusKm: 2e9, // audible from very far
    maxGain: 0.6,
    gainCurve: 'logarithmic',
    stems: ['sun/drone-01.wav', 'sun/texture-01.wav'],
    textureFile: 'sun.jpg',
    emissive: true,
    color: '#FDB813',
  },
  {
    id: 'mercury',
    name: 'Mercury',
    type: 'planet',
    radiusKm: 2_439,
    audibilityRadiusKm: 5e7,
    maxGain: 0.5,
    gainCurve: 'logarithmic',
    stems: ['mercury/tone-01.wav'],
    textureFile: 'mercury.jpg',
    color: '#8C7E6D',
  },
  {
    id: 'venus',
    name: 'Venus',
    type: 'planet',
    radiusKm: 6_051,
    audibilityRadiusKm: 8e7,
    maxGain: 0.55,
    gainCurve: 'logarithmic',
    stems: ['venus/pad-01.wav', 'venus/texture-01.wav'],
    textureFile: 'venus.jpg',
    color: '#C4A882',
    atmosphereColor: '#E8C080',
  },
  {
    id: 'earth',
    name: 'Earth',
    type: 'planet',
    radiusKm: 6_371,
    audibilityRadiusKm: 1e8,
    maxGain: 0.6,
    gainCurve: 'logarithmic',
    stems: ['earth/pad-01.wav', 'earth/melody-01.wav'],
    textureFile: 'earth.jpg',
    color: '#4B7BE5',
    atmosphereColor: '#6CA6FF',
  },
  {
    id: 'mars',
    name: 'Mars',
    type: 'planet',
    radiusKm: 3_389,
    audibilityRadiusKm: 7e7,
    maxGain: 0.5,
    gainCurve: 'logarithmic',
    stems: ['mars/drone-01.wav', 'mars/texture-01.wav', 'mars/texture-02.wav'],
    textureFile: 'mars.jpg',
    color: '#C1440E',
  },
  {
    id: 'jupiter',
    name: 'Jupiter',
    type: 'planet',
    radiusKm: 69_911,
    audibilityRadiusKm: 5e8,
    maxGain: 0.8,
    gainCurve: 'logarithmic',
    stems: ['jupiter/bass-01.wav', 'jupiter/pad-01.wav', 'jupiter/texture-01.wav'],
    textureFile: 'jupiter.jpg',
    color: '#C88B3A',
    atmosphereColor: '#D4A65A',
  },
  {
    id: 'saturn',
    name: 'Saturn',
    type: 'planet',
    radiusKm: 58_232,
    audibilityRadiusKm: 4e8,
    maxGain: 0.75,
    gainCurve: 'logarithmic',
    stems: ['saturn/pad-01.wav', 'saturn/shimmer-01.wav', 'saturn/drone-01.wav'],
    textureFile: 'saturn.jpg',
    color: '#E8D191',
    atmosphereColor: '#F0DCA0',
    hasRings: true,
    ringInnerRadiusKm: 74_500,
    ringOuterRadiusKm: 140_220,
  },
  {
    id: 'uranus',
    name: 'Uranus',
    type: 'planet',
    radiusKm: 25_362,
    audibilityRadiusKm: 3e8,
    maxGain: 0.6,
    gainCurve: 'logarithmic',
    stems: ['uranus/tone-01.wav', 'uranus/texture-01.wav'],
    textureFile: 'uranus.jpg',
    color: '#73C2CB',
    atmosphereColor: '#A0E0E8',
    hasRings: true,
    ringInnerRadiusKm: 41_837,
    ringOuterRadiusKm: 51_149,
  },
  {
    id: 'neptune',
    name: 'Neptune',
    type: 'planet',
    radiusKm: 24_622,
    audibilityRadiusKm: 3e8,
    maxGain: 0.6,
    gainCurve: 'logarithmic',
    stems: ['neptune/pad-01.wav', 'neptune/drone-01.wav'],
    textureFile: 'neptune.jpg',
    color: '#3E54E8',
    atmosphereColor: '#5070FF',
  },
];

// Shared audio pools for Phase 2+ bodies without unique stems
export const AUDIO_POOLS: Record<string, string[]> = {
  'dwarf-planet': ['pools/dwarf-planet/tone-01.wav', 'pools/dwarf-planet/texture-01.wav'],
  'rocky-moon': ['pools/rocky-moon/tone-01.wav'],
  'icy-moon': ['pools/icy-moon/shimmer-01.wav', 'pools/icy-moon/texture-01.wav'],
};
