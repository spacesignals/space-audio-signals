...
export class SolarSystem {
      ...
      if (config.emissive) {
        ...
        if (config.textureFile) {
          this.textureLoader.load(
            ...
            undefined,
            () => {
              // File texture failed — procedural texture (if any) stays
            }
          );
        }
      }
  ...
  private createStarfield(): void {
    // Milky Way skybox — procedural equirectangular panorama on inverted sphere
    const skyTex = generateSkyboxTexture();
